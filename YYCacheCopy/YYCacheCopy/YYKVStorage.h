...
@property (nonatomic) int modTime;                          ///< modification unix timestamp 修改unix时间戳
@property (nonatomic) int accessTime;                       ///< last access unix timestamp 上次访问 unix的时间戳
@property (nullable, nonatomic, strong) NSData *extendedData; ///< extended data (nil if no extended data) 扩展数据（如果没有的话为nil）
@end
 ...
 * If you want to store large number of small datas (such as contacts cache), 
   use YYKVStorageTypeSQLite to get better performance. 适合存储大量小的数据
 * If you want to store large files (such as image cache),
   use YYKVStorageTypeFile to get better performance. 适合存储大的文件，若果图片缓存
 * You can use YYKVStorageTypeMixed and choice your storage type for each item. 混合
...
/** 保存一个item或者更新一个item使用这个key，如果它已经被释放掉了
 ...
 item的value将会被保存进文件系统如果这个item的文件名不为空，除此以外它将会保存进sqlite
...
/**
 ...
 这个方法将会保存进键值对到sqlite 如果是文件类型的，这个方法将会返回失败
