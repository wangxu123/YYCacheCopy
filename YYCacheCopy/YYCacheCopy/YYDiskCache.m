...
static NSMapTable *_globalInstanes;
static dispatch_semaphore_t _globalInstancesLock;// 锁，信号量，控制线程的数量

static void _YYDiskCacheInitGlobal() {
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _globalInstancesLock = dispatch_semaphore_create(1);
        _globalInstanes = [[NSMapTable alloc] initWithKeyOptions:NSPointerFunctionsStrongMemory valueOptions:NSPointerFunctionsWeakMemory capacity:0];
    });
}
...
static YYDiskCache *_YYDiskCacheGetGlobal(NSString *path) {
    ...
    id cache = [_globalInstanes objectForKey:path];
...
static void _YYDiskCacheSetGlobal(YYDiskCache *cache) {
    ...
    _YYDiskCacheInitGlobal();
    dispatch_semaphore_wait(_globalInstancesLock, DISPATCH_TIME_FOREVER);
    [_globalInstanes setObject:cache forKey:cache.path];
    dispatch_semaphore_signal(_globalInstancesLock);
}
...
- (void)_trimRecursively {
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)((int64_t)_autoTrimInterval * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
...
- (NSString *)_filenameForKey:(NSString *)key {
    ...
    if (_customFileNameBlcok) filename = _customFileNameBlcok(key);
...
- (instancetype)initWithPath:(NSString *)path inlineThreshold:(NSUInteger)threshold {
    ...
    _queue = dispatch_queue_create("com.libireme.cache.disk", DISPATCH_QUEUE_PRIORITY_DEFAULT);
...
- (void)setObject:(id<NSCoding>)object forkey:(NSString *)key {
    ...
    NSData *extendedData = [YYDiskCache getExtendDataFromObject:object];
    NSData *value = nil;
...
- (void)totalCountWithBlock:(void (^)(NSInteger totalCount))block {
    ...
    dispatch_async(_queue, ^{
        __strong typeof(_self) self = _self;
        NSInteger totalCoast = [self totalCost];
        block(totalCoast);
    });
}
...
- (void)totalCostWithBlock:(void (^)(NSInteger))blcok {
    if (!blcok) return;
    __weak typeof(self) _self = self;
    dispatch_async(_queue, ^{
        __strong typeof(_self) self = _self;
...
+ (NSData *)getExtendDataFromObject:(id)object {
